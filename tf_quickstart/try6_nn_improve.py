"""
Summary:
    - Default optimizer Adagrad works great for convex problems.
    - Scale feature to [-1; 1]. If linear scale is bias, try others (ex: log)
    -
"""
...
def train_nn_regression_model(
    ...
    print(
        "Final RMSE (on training data):   %0.2f" % training_root_mean_squared_error)
    print(
        "Final RMSE (on validation data): %0.2f" % validation_root_mean_squared_error)
...
def normalize(examples_dataframe):
    ...
    processed_features["median_income"] = log_normalize(
        examples_dataframe["median_income"])
    processed_features["total_bedrooms"] = log_normalize(
        examples_dataframe["total_bedrooms"])
    ...
    processed_features["housing_median_age"] = linear_scale(
        examples_dataframe["housing_median_age"])
    ...
    processed_features["rooms_per_person"] = linear_scale(
        clip(examples_dataframe["rooms_per_person"], 0, 5))
    processed_features["total_rooms"] = linear_scale(
        clip(examples_dataframe["total_rooms"], 0, 10000))
...
normalized_training_examples.describe().to_csv("temp.csv")

# Adagrad works great for convex problems.
# For non-convex optimization problems, Adam is sometimes more efficient than
# Adagrad.
